...
def read_spec_todos(user: user_dependency, db: db_dependency, todo_id: int):
    if user is None:
        raise HTTPException(status_code=401, detail="Failed Authentication")
    spec_todo = (
        db.query(Todos)
...
def create_todo(user: user_dependency, db: db_dependency, new_todo: Todo):
    ...
    db.commit()
    return JSONResponse(
        status_code=201, content={"message": "Todo created successfully"}
    )
...
def update_todos(
    ...
    db.commit()
    return JSONResponse(
        status_code=200, content={"message": "Todo updated successfully"}
    )
...
def delete_todos(user: user_dependency, db: db_dependency, todo_id: int):
    ...
    db.commit()
    return JSONResponse(
        status_code=200, content={"message": "Todo deleted successfully"}
    )
